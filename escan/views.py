...
from django.db.models import Sum, Count, Avg, Q, ExpressionWrapper, F, DecimalField
...
from .supabase_helper import upload_image_to_supabase, upload_detection_image
import logging
...
def my_store_dashboard(request):
    ...
    # Helper for revenue after admin commission
    revenue_expr = ExpressionWrapper(F('total_amount') - F('admin_commission'), output_field=DecimalField(max_digits=12, decimal_places=2))

    # ========== CUSTOMER METRICS ==========
    total_customers = Order.objects.filter(store=store, status="Completed").values('customer').distinct().count()
    new_customers_month = Order.objects.filter(store=store, status="Completed", order_date__gte=month_ago).values('customer').distinct().count()
    repeat_customers = Order.objects.filter(store=store, status="Completed")\
        .values('customer')\
    ...
    total_sales = Order.objects.filter(store=store, status="Completed").aggregate(
        total_sales=Sum(revenue_expr)
    )["total_sales"] or 0

    today_sales = Order.objects.filter(store=store, status="Completed", order_date__date=today).aggregate(
        total_sales=Sum(revenue_expr)
    )["total_sales"] or 0

    weekly_sales = Order.objects.filter(store=store, status="Completed", order_date__gte=week_ago).aggregate(
        total_sales=Sum(revenue_expr)
    )["total_sales"] or 0

    monthly_sales = Order.objects.filter(store=store, status="Completed", order_date__gte=month_ago).aggregate(
        total_sales=Sum(revenue_expr)
    )["total_sales"] or 0

    yearly_sales = Order.objects.filter(store=store, status="Completed", order_date__gte=year_ago).aggregate(
        total_sales=Sum(revenue_expr)
    )["total_sales"] or 0

    # ========== STOCK METRICS ==========
    total_stocks = Product.objects.filter(store=store).aggregate(total_stock=Sum('stock'))['total_stock'] or 0
    low_stock_products = Product.objects.filter(store=store, stock__lt=10, stock__gt=0).count()
    out_of_stock_products = Product.objects.filter(store=store, stock=0).count()
    stock_value = Product.objects.filter(store=store).aggregate(total_value=Sum(F('price') * F('stock')))['total_value'] or 0
    ...
    for i in range(7):
        date = today - timedelta(days=i)
        day_sales = Order.objects.filter(store=store, status="Completed", order_date__date=date).aggregate(
            total_sales=Sum(revenue_expr),
            order_count=Count('id')
        )
        weekly_sales_data.append({
    ...
    for i in range(12):
        month_start = (today.replace(day=1) - timedelta(days=30*i))
        month_end = (month_start + timedelta(days=32)).replace(day=1) - timedelta(days=1)
        month_sales = Order.objects.filter(store=store, status="Completed", order_date__range=[month_start, month_end]).aggregate(
            total_sales=Sum(revenue_expr)
        )
        monthly_sales_data.append({
            'month': month_start.strftime('%b %Y'),
    ...
    # ========== TOP PRODUCTS ==========
    top_products = Product.objects.filter(store=store, order__status="Completed")\
        .annotate(total_sold=Sum('order__quantity'), total_revenue=Sum(ExpressionWrapper(F('order__total_amount') - F('order__admin_commission'), output_field=DecimalField(max_digits=12, decimal_places=2))))\
        .order_by('-total_sold')[:5]
...
def validation_list(request):
    validations = (
        StoreValidation.objects
        .exclude(store_owner__is_staff=True)
        .exclude(store_owner__is_superuser=True)
        .select_related('store_owner')
        .order_by('-created_at')
    )

    return render(
        request,
        'escan/Admin/E-commerce/store_validation.html',
        {
...
@login_required
@user_passes_test(is_admin)
def admin_monthly_revenue_api(request):
    month = request.GET.get("month")  # YYYY-MM
    store_id = request.GET.get("store_id")  # optional
    ...
    if month:
        year, m = month.split("-")
        orders = orders.filter(order_date__year=year, order_date__month=m)

    data = (
        orders.annotate(month=TruncMonth("order_date"))
        .values("month")
        .annotate(total=Sum("admin_commission"))
        .order_by("-month")
    )
...
@login_required
@user_passes_test(is_admin)
def admin_revenue_logs(request):
    month = request.GET.get('month')   # YYYY-MM
    store_id = request.GET.get('store')

    orders = Order.objects.filter(status='Completed').select_related('store')
    ...
    orders = orders.annotate(
        net_store_amount=ExpressionWrapper(
            F('total_amount') - F('admin_commission'),
    ...
    return render(request, 'escan/Admin/E-commerce/revenue_logs.html', {
        'orders': orders,
        'stores': stores
    })
...
@login_required
@user_passes_test(is_admin)
def export_admin_revenue_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="admin_revenue_logs.csv"'

    writer = csv.writer(response)
    writer.writerow([
        'Date', 'Store', 'Order ID',
        'Gross Amount', 'Admin Commission', 'Net Store Revenue'
    ])
    ...
    for o in orders:
        writer.writerow([
            ...
            o.total_amount,
            o.admin_commission,
            o.total_amount - o.admin_commission
        ])
...
#         .values('month')
#         .annotate(total_commission=Sum('admin_commission'))
#         .order_by('-month')
#     )
...
def a_banana_disease(request):
    ...
    if request.method == "POST":
            ...
            if form.is_valid():
                ...
                image_url, image_bytes = upload_detection_image(image, user)
                ...
                try:
                    ...
                    if isinstance(result_api, dict):
                        result = result_api.get("prediction", "Unknow Data")
                        confidence = float(result_api.get("confidence", 0.0))
...
def a_predict_from_camera(request):
    ...
    try:
        ...
        print("❌ Error in predict_disease_from_camera:", e)
        return JsonResponse(
...
def a_banana_variety(request):
    ...
    CLASS_DESCRIPTIONS = {
        ...
        'Sabri Kola': {
            'description': 'Ang Sabri Kola ay hindi gaanong kilalang barayti na may katamtaman hanggang malaking bunga.',
            'Classification': 'Dual-purpose na saging.',
            'Origin': 'Itinatanim sa ilang bahagi ng Bangladesh at India.',
            'HarvestPeriod': '11-13 buwan matapos itanim.'
        },
        'Unknow Data': {
            'description': 'Limitado pa ang impormasyon tungkol sa barayting ito.',
            'Classification': 'Hindi pa tiyak.',
            'Origin': 'Hindi pa natutukoy.',
            'HarvestPeriod': 'Hindi pa matukoy.'
        }
    }
    ...
    if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(request, "escan/Admin/A_Scan/a_banana_variety_result.html",
                    {
                        "result": result,
                        "confidence": round(confidence, 2),
                        "prediction_time": prediction_time,
...
def banana_disease(request):
    ...
    if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    "escan/Farmer/Scan/banana_disease_result.html",
                    {
                        "result": result,
                        "confidence": round(confidence, 2),
                        "prediction_time": prediction_time,
...
def banana_variety(request):
    ...
    if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    "escan/Farmer/Scan/banana_variety_result.html",
                    {
                        "result": result,
                        "confidence": round(confidence, 2),
                        "prediction_time": prediction_time,
    ...
    return render(
        request,
        "escan/Farmer/Scan/banana_variety.html",
        {"form": form},
    )
